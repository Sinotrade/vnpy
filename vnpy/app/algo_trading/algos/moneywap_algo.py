...
class MoneywapAlgo(AlgoTemplate):
    ...
    default_setting = {
        ...
        "time_line": "09:00",
        "traded": 0,
        "offset": [Offset.NONE.value, Offset.CLOSETODAY.value],
    }
    ...
    def __init__(self, algo_engine: BaseEngine, algo_name: str, setting: dict):
        ...
        order_price = tick.limit_down if tick.limit_down else (tick.last_price * 0.9)

        self.order_volume = 1 if order_price > 15 else int(15 // order_price) + 1

        if self.qty == 0:
            order_price = (
                tick.limit_down if tick.limit_down else (tick.last_price * 0.9)
            )
            print(order_price, tick.limit_down)
            self.volume = int(
                self.total_amt // 1000 // order_price // self.order_volume
            )  # 總數量
        else:
    ...
    def on_timer(self):
        ...
        if self.direction == Direction.LONG:
            if tick.ask_price_1 <= self.price:
                self.buy(self.vt_symbol, self.price, order_volume, offset=self.offset)
                self.display_volume += self.order_volume
                self.purchased_amount += self.order_volume * tick.last_price * 1000
            else:
                self.price = tick.ask_price_5
        else:
            if tick.bid_price_1 >= self.price:
                self.sell(self.vt_symbol, self.price, order_volume, offset=self.offset)
                self.display_volume += self.order_volume
                self.purchased_amount += self.order_volume * tick.last_price * 1000
            else:
